"""
# ASR (zero-shot) + phonetic edit distance retrieval, batched
python infer_qwen2.5_omni_batch.py \
    --speaker P001 \
    --domain roads \
    --batch_size 8

# ASR adaptation (1-shot) + phonetic edit distance retrieval, batched
python infer_qwen2.5_omni_batch.py \
    --speaker P001 \
    --domain roads \
...
class Qwen25OmniASRPipeline:
    """ASR using Qwen2.5-Omni, with batched generation support."""
    ...
    def __init__(self, config: Optional[ASRConfig] = None):
        ...
        # Batched generation needs left padding: with right padding the varying sequence
        # lengths would misalign where generation starts.
    ...
    def _transcribe_batch(self, batch_messages: List[list], max_new_tokens: int) -> List[str]:
        """Forward/generate over a batch of conversations in one pass."""
        ...
        # With left padding every sample has the same prompt length -> slice them all at once.
    ...
    def run_asr_batch(
        ...
        """Transcribe a list of audios in batches. When ref_audio is given, the same
        1-shot asr_adaptation is applied to every sample."""
        audios = [self._normalize_audio_input(a) for a in audios]
        if ref_audio is not None:
            ref_audio = self._normalize_audio_input(ref_audio)
        batch_messages = [
            self._build_messages(audio, ref_audio=ref_audio, reference_text=reference_text)
            for audio in audios
        ]
        results = self._transcribe_batch(batch_messages, self.config.max_new_tokens)
...
if __name__ == "__main__":
    ...
    for batch_rows in chunked(data, args.batch_size):
        batch_audio = [
            load_audio_16k(f"L2-KPNS/data/audio/{args.speaker}/" + row["audio_path"])
            for row in batch_rows
        ]
        batch_out = asr.run_asr_batch(
            audios=batch_audio,
            ref_audio=asr_ref_audio,
        ...
        for row, out in zip(batch_rows, batch_out):
            ...
            print(f"[{done}/{len(data)}] asr_result='{out}' -> retr_entities={entity_list_text}")
